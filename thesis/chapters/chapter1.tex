...
\subsubsection{Supervised learning model definition}
...
In common machine learning problems, it is important to be on guard against overfitting. In some cases a philosophical solution comes from the \textit{Occam's Razor}, which states that best solutions are also the simplest on the basis that they are capturing more fundamental aspects. \textit{Regularization} is an application of this principle on ERM: instead of searching the model that best fits the training data, a penalty based on the model complexity is added. The ERM rule becomes:
...
%TODO stability, l1, l2, l1 for feature selection
